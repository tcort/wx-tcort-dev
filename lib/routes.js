...
function routerFactory(options = {}) {
    ...
    router.get('/', function (req, res, next) {
        ...
        req.db.eventGetLatest('up', 'a84041d871888363', (err, evt) => {
            ...
            const temperature = evt.json.object.TempC_SHT.toFixed(2);
            const humidity = evt.json.object.Hum_SHT.toFixed(2);
            const timestamp = moment(evt.time).tz('America/Toronto').format('YYYY-MM-DD HH:mm:ss');

            res.render('index', { temperature, humidity, timestamp }, function (err, html) {
                if (err) {
                    return next(err);
                }
                res.send(html);
