...
function routerFactory(options = {}) {
    ...
    router.get('/', function (req, res, next) {
        ...
        req.db.eventGetLatest('up', 'a84041d871888363', (err, evt) => {
            ...
            const temperature_f = wx.celsiusToFahrenheit(temperature_c);
            ...
            const heat_index_f = wx.heatIndexFahrenheit(temperature_f, relative_humidity);
            ...
            req.db.eventStatsAll('up', 'a84041d871888363', '$.object.TempC_SHT', function (err, stats) {
                ...
                [ 'lastDay', 'lastWeek', 'lastMonth', 'allTime' ].forEach(period => {
                    [ 'min', 'avg', 'max' ].forEach(key => {
                        stats[period][`${key}_c`] = stats[period][key].toFixed(2);
                        stats[period][`${key}_f`] = wx.celsiusToFahrenheit(stats[period][key]).toFixed(2);
                        delete stats[period][key];
                    });
                });
