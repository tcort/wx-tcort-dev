...
class DB {
    ...
    eventStats(type, devEui, path, since, callback) {
        const sql = 'SELECT round(min(json_extract(json, ?)), 2) as min, round(avg(json_extract(json, ?)), 2) as avg, round(max(json_extract(json, ?)), 2) as max FROM events WHERE type = ? AND devEui = ? AND time > ?;';
